...
=========================================
"Control flow" means the choice of WHICH lines run, and in what order.
The operators below produce True or False values. Python uses those values
...
# IMPORTANT: one "=" assigns a value, two "==" compare two values.
...
# ---------------------------------------------------------------------------
# Python runs the FIRST branch whose condition is True. It then skips the rest.
# Indentation (4 spaces) tells Python which lines belong to a branch.

score = 75
if score >= 90:
    print("Grade: A")
elif score >= 70:          # Python checks this only if the line above was False
...
#   not A    -> changes True to False and False to True
...
logged_in = False
if not logged_in:                            # change a condition to its opposite
...
# If you are not sure, add brackets so the line reads the way you want.
country = "UK"
...
#   - "and": if the first part is False, Python skips the rest (result False).
#   - "or" : if the first part is True,  Python skips the rest (result True).

def check():
    print("  check() ran")
    return True

print("False and check():", False and check())   # Python does not call check()
print("True or check():", True or check())        # Python does not call check()

# This helps you avoid errors:
name = ""
# If name is empty, the left side is False. Python then does not evaluate
# name[0], so it does not raise an "index out of range" error.
if name != "" and name[0] == "A":
    print("Name starts with A")
else:
    print("No name, or it does not start with A")
...
# Use "==" almost always. Use "is" when you compare a value to None.
...
# ---------------------------------------------------------------------------
# An if-condition does not have to be a real True or False. Some values act
# like False:
#   Falsy: False, None, 0, 0.0, "" (empty string), [] {} () (empty containers)
#   Truthy: almost every other value

items = []
if items:                       # an empty list is falsy
    print("Cart has items")
else:
    print("Your cart is empty")

username = "neo"
if username:                    # a non-empty string is truthy
...
user_age = 19
has_license = True
print("Practice 1 (expect True):", user_age >= 18 and has_license)
